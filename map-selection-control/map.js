var SelectableUsMap = (function() {
  function SelectableUsMap() {
    this.log("create map!");
  ...
  // Add and remove states from State List
  SelectableUsMap.prototype.toggleState = function toggleState(state) {
    // Get States index in array
    var indexOfItem = $.inArray(state, this.selectedStates);
    // Remove it if it exists, or add it if it doesn't
    if (indexOfItem !== -1) {
      this.selectedStates.splice(indexOfItem, 1);
    } else {
  ...
  // Set unusable states
  SelectableUsMap.prototype.setDisabledStates = function setDisabled(states) {
    var currentState;
    $.each(states, function(i, s) {
      currentState = "#" + s;
      $(currentState).css("pointer-events", "none");
      $(currentState).css("fill", "#bbb");
    });
    this.log("Unusable List: " + states);
  };

  SelectableUsMap.prototype.selectState = function hilight(index, state) {
    if (index && !state) {
      state = this.selectedStates[index];
    }
    state = "#" + state;
    $(state).data("clicked", "yes");
    $(state).css("fill", this.primaryColor);
  };

  SelectableUsMap.prototype.setSelected = function setSelectedStates(states) {
    this.selectedStates = states;
    $.each(states, this.selectState.bind(this));
    this.log("Starting List: " + states);
  };

  SelectableUsMap.prototype.getSelected = function getSelectedStates() {
    return { selectedStates: this.selectedStates };
  };

  /*
   * Decisions Form Specific API methods:
   */

  /**
   * @param {Object} value for the map to render
   * @param {string[]} value.selectedStates which are already selected
   * @param {string[]} value.disabledStates which cannot be selected
   * @param {boolean} value.debug true to allow console logging.
   */
  ...
  /**
   * @param {JQuery<HTMLElement>} hostJqElement jquery element to append custom content into
   * @param component
   */
  SelectableUsMap.prototype.initialize = function initialize(
    hostJqElement,
    component
  ) {
    this.log("Initializing US map control.");
    this.host = hostJqElement;
    var map = this;

    /*** render map DOM ***/
    renderMap(hostJqElement);

    /*** add handlers ***/

    // Define beviour when a state is hovered over
    $("path, circle").hover(function(e) {
      //Show details box
      $("#details").css("display", "block");
      $("#details").html($(this).data("info"));

      // Show Primary Color When Hovered Over
      $(this).css("fill", map.primaryColor);
    });

    // Define behviour when a state is clicked
    $("path, circle").click(function(e) {
      //Toggle class to highlight states, and run function to toggle add or remove state from state list
      ...
      map.toggleState($(this).attr("id"));
    });

    // Define behaviour when the mouse leaves the states area
    $("path, circle").mouseleave(function(e) {
      // Hide details box when leaving state
      $("#details").css("display", "none");
      // If  the state isnt clicked, then change color back after leaving state
      if ($(this).data("clicked") != "yes") {
        $(this).css("fill", "");
      }
    });

    // Define behavior when the cursor moves
    $(document)
      .mousemove(function(e) {
        // Keep details box near cursor
        var details = $("#details");
        details.css("position", "absolute");
        details.css("padding", "0.5rem");
        details.css("background", "white");
        details.css("border", "solid 1px #555");
        details.css("box-shadow", "1px 1px 2px rgba(0,0,0, 0.3)");
        details.css("top", e.offsetY - details.height() - 30);
        details.css("left", e.offsetX - details.width() / 2);
      })
      .mouseover();
  };
  SelectableUsMap.prototype.resize = function(height, width) {
    if (this.host && height && width && height > 0 && width > 0) {
      this.host.css({ width: width, height: height });
    }
  };

  SelectableUsMap.prototype.log = function(message) {
    if (!this.debug) {
      return;
    }
    console.log(message);
  ...
  function renderMap(element) {
    var mapSvgString = [
      ...
      '<circle id="DC" data-info="<div>Washington DC</div>" fill="#D3D3D3" stroke="#FFFFFF" stroke-width="1.5" cx="975.3" cy="351.8" r="5"/>',
      "</g>",
      "</g>",
    ...
    element.html(mapSvgString);
  }

  return SelectableUsMap;
})();
